...
@MainActor
final class NottiRecordRecallTests: XCTestCase {
    ...
    func testMerge_semanticExcludesAnchorDuplicates_andReportsBoundary() {
        ...
        let s1 = rec("语义1")
        let out = NottiRecordRecall.merge(anchors: [a1, a2], semantic: [a1, s1], cap: 20)
    ...
    func testMerge_capTruncates_andClampsBoundary() {
        ...
        let out = NottiRecordRecall.merge(anchors: anchors, semantic: semantic, cap: 5)
    ...
    func testRecall_mergesAnchorsAndSemantic_excludesEncryptedFromCandidates() async {
        ...
        let recaller = NottiRecordRecall(engine: stub, anchorCount: 1, semanticLimit: 5, mergedCap: 20)
