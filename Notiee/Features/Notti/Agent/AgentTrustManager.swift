...
final class AgentTrustManager: ObservableObject {
    ...
    func setLevel(_ level: AgentTrustLevel) {
        currentLevel = level
        settingsStore.saveString(level.rawValue, forKey: UDK.nottiAgentTrustLevel)
    }

    private func loadSavedSettings() {
        let raw = settingsStore.loadString(forKey: UDK.nottiAgentTrustLevel, defaultValue: AgentTrustLevel.standard.rawValue)
