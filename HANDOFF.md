...
- Deferred the Notti scroll-following state, Today geometry measurements, and record-detail scroll-offset state until the next main-queue turn. These were the three view-update paths responsible for SwiftUI's `Publishing changes from within view updates is not allowed` diagnostics; redundant writes are also suppressed.
- Replaced stored `[String: Any]` values in Sendable Agent call/result/undo models with JSON `Data` storage while preserving dictionary-facing APIs. Agent tools and their registry are now explicitly main-actor-owned, matching `AgentExecutor` and the record/calendar managers and removing the delete-tool isolation warnings.
- Moved all mutable AVFoundation session/device/output state behind one serial-queue-owned `SessionState`, snapshots flash mode before dispatch, and publishes camera capabilities back on the main actor. This removes the CameraManager Sendable-closure warnings without marking the whole manager unchecked.
- Marked the AI pipeline record-access protocol main-actor-owned, removed its redundant `await`, updated microphone permission requests to `AVAudioApplication`, made note device-name detection nonisolated, removed the RecordMigrator immutable-variable warning, and clarified the targeted UserDefaults/Keychain Sendable boundaries for direct and backend AI processing. `BackendAIProcessingService` remains free-target only; the direct service is shared source and selected by Notiee+.
- Added the missing shared `AccentColor.colorset` using the canonical Notiee green. The asset catalog is consumed by both app targets (and their shared catalog consumers), so Xcode no longer reports a missing global accent color.
- Low-memory verification only: all 202 app Swift files parse; the NewUIPreview core type-checks with `strict-concurrency=complete` under both free and `NOTIEE_PLUS`; CameraManager and the Agent value protocol type-check independently with the same strict settings; four preview XCTest sources type-check against an emitted testable module; the existing free and Plus Notti executable harnesses pass; project/localization plist lint, asset JSON parsing, localization key parity, and `git diff --check` pass.
- Full-source strict type-checking reached the repository's `@TaskLocal` macro but the Swift plugin server cannot run inside this sandbox; the required unsandboxed command was rejected by the approval service. Xcode, Simulator, and `xcodebuild test` were intentionally not used because the user reported insufficient memory. The generic project-level `Update to recommended settings` item was not applied because the screenshot does not expose Xcode's proposed build-setting diff; do not change project-wide recommendations blindly.

### 2026-08-06 — NewUIPreview Notti and Records demo redesign _(both targets; experimental preview only)_

- Follow-up low-memory audit fixed four runtime-facing issues that static parsing did not cover: canceled stream/focus tasks now exit instead of continuing after cancellation, repeated preview toasts no longer clear each other early, loading or starting a conversation restores bottom-following state, and the near-bottom calculation now includes SwiftUI scroll content insets. Two async cancellation regression tests were added, and the missing Simplified/Traditional Chinese `Agent` localization key was filled so all three catalogs have matching key sets.
- Follow-up verification avoided Xcode, Simulator, and `xcodebuild test`: standalone macOS executables compiled the real Notti state for both free and Plus configurations and passed the async send/wait/stop/regenerate/session assertions; all NewUIPreview sources type-check against the iPhoneOS 18 SDK in both configurations; and all five focused XCTest files type-check against an emitted testable preview module. Strict concurrency additionally reports only pre-existing warnings in `NoteRecord`, `CuratedModelCatalog`, and production Agent types, outside this preview change.
- Rebuilt the preview-only Notti surface as a full-screen mock conversation: autofocus composer, deterministic shortcuts/history, multi-line send/stop, target-specific model fixtures, one-shot web search, conversation-scoped Agent mode, Markdown chunk streaming, near-bottom auto-scroll, Copy, and replace-in-place Regenerate. User bubbles continue to use the configured Notiee theme color. Reduce Motion uses coarse response chunks and a non-animated streaming cursor.
- Added preview-owned Notti message/conversation state with cancellable streaming tasks and deterministic test hooks. The free target derives its fixture models from `CuratedModelCatalog`; Notiee+ uses an isolated BYOK-style fixture list under `#if NOTIEE_PLUS`. No production Notti transport, repository, memory, persistence, or backend code changed.
- Added Records scopes for All, Favorites, Uncategorized, Today's Captures, Pending, Trash, custom folders, and linked events. The large navigation title now opens the native scope menu; Folder and Linked Events are nested submenus, current selections show a checkmark, and choosing a scope updates the title and requests a scroll to the masonry top. New/Manage Folder remain preview-only notice actions.
- Expanded deterministic record fixtures and implemented scope + source + search intersection semantics. All excludes deleted records, Trash contains only deleted records, and Uncategorized means no folder even when an event is linked. The internal Notti-generated folder remains hidden from the folder submenu and accessible through the existing Notti source filter.
- Registered `NewUIPreviewNottiState.swift` in both app source phases and `NewUIPreviewNottiStateTests.swift` in the test target. Added focused Notti state tests and expanded Records scope/intersection tests. Added all new copy to English, Simplified Chinese, and Traditional Chinese localization files.
- Verification used the low-memory path requested by the user: all NewUIPreview Swift and test files parse; the complete Notiee and Notiee+ iPhoneOS source lists type-check successfully (with only pre-existing Swift 6 concurrency warnings); all five NewUIPreview XCTest source files type-check against an emitted `@testable` Notiee module; project/localization plist lint and `git diff --check` pass. Simulator XCTest execution and visual screenshots were intentionally skipped because the user said this Mac cannot run local Xcode testing reliably. No commit or push was created.

### 2026-08-06 — Preferred native nested menu for Records folders _(both targets; discussion only)_

- The preferred folder entry is an iOS-native nested menu like the supplied Files screenshot: navigation title trigger (`记录`) -> primary scope menu -> `文件夹` submenu. This is three interaction levels, not hierarchical/nested folder data; the existing flat `CustomFolder` model remains unchanged.
- The primary menu owns All Records and the smart collections (Favorites, Unclassified, Today's Records, Pending, Trash). The folder submenu lists ordinary user folders with a selected checkmark, plus New Folder and Manage Folders commands; selecting a folder closes the menu stack and changes the navigation title to that folder's name.
- Keep event-linked collections in a sibling `关联日程` submenu rather than nesting them under folders. The internal Notti-generated folder should not duplicate the visible `来自 Notti` source filter.
- Native nested menus are appropriate for normal folder counts. If the list becomes long, show recently used folders plus an `全部文件夹…` command that opens a searchable management sheet; do not let an unbounded submenu become the only access path.
- No application source, persistence, localization, project membership, or runtime behavior changed in this discussion.

### 2026-08-06 — Records scope menu and legacy-folder direction _(both targets; discussion only)_

- The large `记录` navigation title can become the current-scope selector. Its primary smart collections are All Records, Favorites, Unclassified, Today's Records, Pending, and Trash; the title changes to the selected scope and returns to `记录` for All Records.
- Preserve the existing folder data model and semantics. A record has one optional `folderID`; folders remain exclusive archive locations with create/rename/delete/move behavior, while the preview's source pills and card metadata are filters/presentation, not a new multi-value tag system. No folder-to-tag migration should occur.
- Put user-created folders behind a `文件夹` group or secondary folder chooser in the title selector. Large folder collections should use a searchable sheet rather than an endlessly nested menu. Selecting a folder filters by `folderID` and shows the folder name as the page title.
- Keep query-based smart collections distinct from archive locations: favorite/today/pending states may overlap with any folder, and Trash is a lifecycle scope rather than a folder. Keep event-linked records in a separate `日程` group because `eventID` is independent of `folderID`.
- Retain source/type pills below search (`照片`, `录音`, `文字`, `来自 Notti`) as a second-stage filter within the selected scope. A folder-looking metadata chip on a card may be tappable as a shortcut, but it must still be described and edited as a folder.
- Folder management remains required in the new UI: create, rename, delete with records becoming unclassified, and moving a record into/out of a folder. It can live in a folder chooser and detail metadata menu rather than occupying the Records overview.
- No application source, persistence, localization, project membership, or runtime behavior changed in this discussion.

### 2026-08-06 — Confirmed Notti reference-UI interaction scope _(both targets; discussion only)_

- The composer plus menu should contain model switching, a per-message web-search control, and Agent mode. Thinking strength remains a separate composer control rather than being folded into Agent mode.
- Read-aloud and sharing are out of scope. Completed assistant responses need only visible Copy and Regenerate actions; Regenerate should replace the latest response for the same user turn rather than append a duplicate branch.
- User-message bubbles continue to use the app-selected theme color. The implementation must validate readable foreground contrast across available theme colors and light/dark appearances.
- The desired progressive answer effect should ultimately be true transport streaming, paired with buffered UI updates so Markdown does not visibly reflow on every token. Free streaming requires the self-hosted backend path; Plus streaming requires its direct BYOK vendor adapters, while the conversation UI remains shared.
- Web search and Agent mode must remain distinct policies: search-only must not silently enable Agent's broader/mutating tools. Active modes need to remain visible near the composer after the plus menu closes.
- No application source, backend, localization, persistence, project membership, or runtime behavior changed in this discussion.

### 2026-08-06 — Mapped the supplied reference UI to Notti behavior _(both targets; design investigation only)_

- The four 1320x2868 references define a full-screen, content-first Notti composer: close at top left; centered Notti/conversation title; contextual new-conversation, history, and overflow controls at top right; a quiet empty state with three prompt shortcuts; a persistent bottom composer that expands above the keyboard; right-aligned user bubbles; unframed Markdown assistant answers; a scroll-to-bottom affordance; and visible answer actions.
- Existing one-to-one mappings are conversation close/persistence, contextual title generation, new conversation, searchable/deletable history, prompt shortcuts, send/stop, thinking-level selection, Plus model selection, Agent mode/timeline/confirmations, citations opening records, privacy notice, tier/paywall and session-limit states. The reference label `Quick` should map to thinking level, not Agent; Agent remains a distinct mode and must have an explicit active/locked state.
- New product capabilities implied by the references but absent from current Notti are attachment intake behind the plus button, microphone speech input, text-to-speech playback, answer sharing, and answer regeneration. Copy exists today only through a context menu and can become a visible action. Attachments and microphone controls must stay absent/disabled until their data, permission, transport, persistence, and privacy contracts exist.
- Recommended placement: the overflow menu owns rename/delete/privacy and low-frequency settings; Plus-only model selection can live in the composer mode menu alongside thinking level while retaining compile-time isolation; Agent is exposed in the composer action/mode surface and changes the visible mode state; citations and the Agent tool timeline remain inline below the relevant answer. The existing AI/privacy disclaimer, injection warning, destructive confirmation, free-tier upsell, and session-limit recovery must remain even though the references do not show them.
- No product source, localization, project membership, backend, persistence, or target-specific behavior changed. No build or test was required for this design-only mapping.

### 2026-08-06 — Assessed NewUIPreview migration readiness _(both targets; investigation only)_

- NewUIPreview is mature enough to serve as the visual/interaction reference for a production migration, but it is not a drop-in production shell. It still owns `NewUIPreviewFixtures`, scenario-driven mock state, preview-only image assets, and a lab exit/scenario toolbar rather than `NotieeStore` and production navigation.
- Release-blocking integrations remain: the Notti composer discards submitted text and has no persistent `NottiViewModel`; camera/audio pulls only show preview toasts; todo/schedule modules use mock rows without detail/toggle actions; Records lacks real folders, favorites, trash, creation, mutation, retry, and authenticated encrypted-record access; detail edit/encrypt/delete/append/emergence actions are placeholders; account/settings access and hardware Camera Control routing are absent.
- The approved production plan at `docs/superpowers/plans/2026-07-29-today-2-context-dashboard.md` remains entirely unchecked. It provides the right migration boundary: port the visual components, introduce real dashboard/resolver adapters, preserve existing Today/Records/detail behavior, let the root own one persistent Notti view model, and migrate legacy `capture`/`spark` launch defaults before switching the root.
- Static syntax parsing passes for all NewUIPreview and focused NewUIPreview test sources. Existing tests cover mock state, layout geometry, record projection/privacy, and presentation details; they do not cover real-store integration, production navigation, mutations, camera lifecycle, Notti transport/state persistence, or end-to-end migration. No product source, localization, project membership, backend, or target-specific behavior changed in this assessment.
